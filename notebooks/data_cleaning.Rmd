...
race_2020 <- race2020[-c(1),] %>%
  ...
  mutate(MinorityRate = 1 - (as.numeric(as.character(P1_001N)) - as.numeric(as.character(P1_003N)))/ as.numeric(as.character(P1_001N))) %>% mutate(TractID=substr(GEO_ID,10,20)) %>% select(TractID, MinorityRate)
...
race_19 <- race19 %>%
  filter(COUNTY %in% c("Fulton", "DeKalb", "Gwinnett", "Clayton", "Cobb")) %>% 
  mutate("MinorityRate" = 1 - as.numeric(as.character(MinorityPercent))/100) %>%
...
### test how many unique tract regions we have
#test=data20$TractID
#length(unique(test)) #318

write.csv(data20,"../datasets/data2020.csv")

print("Eviction data of year 2019 saved!")
print(head(data19))
print("Eviction data of year 2020 saved!")
print(head(data20))
